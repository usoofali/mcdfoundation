                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <flux:heading size="lg"
                                class="mt-1 font-semibold text-base sm:text-lg text-neutral-900 dark:text-white">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="rounded-lg bg-orange-100 p-2 sm:p-3 dark:bg-orange-900/20">
                            <flux:icon name="user-group"
                                class="size-5 sm:size-6 text-orange-600 dark:text-orange-400" />
                        </div>
                        <div class="min-w-0 flex-1">
                            <flux:text class="text-xs sm:text-sm font-medium text-neutral-600 dark:text-neutral-400">
                                Spouses
                            ...
                            @foreach($this->dependents as $dependent)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-neutral-900 dark:text-white">{{ $dependent->name }}
                                        </div>
                                        @if($dependent->notes)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $dependent->eligible ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-neutral-100 text-neutral-800 dark:bg-neutral-700 dark:text-neutral-200' }}">
                                            {{ $dependent->eligible ? 'Eligible' : 'Not Eligible' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-400">
                                        @if($dependent->document_path)
                                            <a href="{{ Storage::url($dependent->document_path) }}" target="_blank"
    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    class="relative z-50 inline-block w-full max-w-lg transform overflow-hidden rounded-2xl border border-neutral-200 bg-white text-left shadow-xl transition-all dark:border-neutral-700 dark:bg-neutral-800 sm:my-8 sm:align-middle">
                        ...
                        <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                <div class="w-full">
                                    ...
                                    <div class="space-y-4">
                                        <flux:input wire:model="form.name" label="Full Name" required autofocus />

                                        <flux:input wire:model="form.nin" label="National ID Number (NIN)"
                                            placeholder="11-digit NIN" required />

                                        <flux:input wire:model="form.date_of_birth" type="date" label="Date of Birth"
                                            required />

                                        <flux:select wire:model="form.relationship" label="Relationship"
                                        ...
                                        <flux:input wire:model="form.document" type="file" label="Document (Optional)"
                                            accept=".pdf,.jpg,.jpeg,.png" />

                                        <flux:textarea wire:model="form.notes" label="Notes (Optional)" rows="3" />
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <flux:modal name="confirm-delete-dependent-{{ $dependent->id }}" focusable class="max-w-lg">
            <div class="space-y-6">
                ...
                <div class="flex justify-end space-x-2 rtl:space-x-reverse">
                    ...
                    <flux:button variant="danger" wire:click="delete({{ $dependent->id }})">
                        {{ __('Delete') }}
                    </flux:button>
                </div>
            </div>
