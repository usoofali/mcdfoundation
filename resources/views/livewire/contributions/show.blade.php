            ...
            <div class="px-6 py-4 border-b border-neutral-200 dark:border-neutral-700">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Receipt:
                    ...
                    <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                        ...
                        <flux:button variant="outline" icon="printer"
                            href="{{ route('contributions.receipt.print', $contribution) }}" target="_blank"
                            class="w-full sm:w-auto">
                            Print Receipt
                        </flux:button>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <dl class="space-y-3">
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                    {{ $contribution->member->full_name }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Registration
                                    Number</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                    {{ $contribution->member->registration_no }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Phone</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                    {{ $contribution->member->phone ?? 'N/A' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Address</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                    {{ $contribution->member->address ?? 'N/A' }}</dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <div>
                        ...
                        <dl class="space-y-3">
                            <div>
                                <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Receipt Number
                                </dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white font-mono">
                                    {{ $contribution->receipt_number }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Contribution Plan
                                </dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                    {{ $contribution->contributionPlan?->label }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Amount</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                    ₦{{ number_format($contribution->amount, 2) }}</dd>
                            </div>
                ...
                <div class="mt-8 pt-8 border-t border-neutral-200 dark:border-neutral-700">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <dl class="space-y-3">
                                <div>
                                    <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Payment
                                        Method</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                        {{ $contribution->payment_method_label }}</dd>
                                </div>
                                @if($contribution->payment_reference)
                                    <div>
                                        <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Payment
                                            Reference</dt>
                                        <dd class="mt-1 text-sm text-gray-900 dark:text-white font-mono">
                                            {{ $contribution->payment_reference }}</dd>
                                    </div>
                                @endif
                                <div>
                                    <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Payment Date
                                    </dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                        {{ $contribution->payment_date->format('F d, Y') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Collected By
                                    </dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                        {{ $contribution->collector?->name ?? 'N/A' }}</dd>
                                </div>
                            </dl>
                        </div>
                        <div>
                            <dl class="space-y-3">
                                <div>
                                    <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Period</dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                        {{ $contribution->period_coverage }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Status</dt>
                                    <dd class="mt-1">
                                        <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                                            @if($contribution->status === 'paid' || $contribution->status === 'verified') bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-neutral-500 dark:text-neutral-400">Submitted by
                                            {{ $contribution->uploader?->name ?? 'Member' }}</p>
                                    </div>
                                </div>
                                <div class="flex space-x-2">
                                    ...
                                    <flux:button variant="outline" size="sm" href="{{ $contribution->receipt_url }}"
                                        target="_blank">
                                        Download
                                    </flux:button>
                                </div>
                    ...
                    <div class="mt-8 pt-8 border-t border-neutral-200 dark:border-neutral-700">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                <dl class="space-y-3">
                                    <div>
                                        <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Verified At
                                        </dt>
                                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                            {{ $contribution->verified_at->format('F d, Y \a\t g:i A') }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Verified By
                                        </dt>
                                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                            {{ $contribution->verifier?->name ?? 'Unknown' }}</dd>
                                    </div>
                                </dl>
                            </div>
                            <div>
                                @if($contribution->verification_notes)
                                    <dl class="space-y-3">
                                        <div>
                                            <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Verification
                                                Notes</dt>
                                            <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                                {{ $contribution->verification_notes }}</dd>
                                        </div>
                                    </dl>
                                @endif
                            </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    ...
                                    <div class="mt-2 text-sm text-yellow-700">
                                        <p>This contribution was submitted by the member and is pending staff verification.
                                        </p>
                                        <p class="mt-1">Submitted on
                                            {{ $contribution->created_at->format('F d, Y \a\t g:i A') }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="mt-8 pt-8 border-t border-neutral-200 dark:border-neutral-700">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <dl class="space-y-3">
                                <div>
                                    <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Created At
                                    </dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                        {{ $contribution->created_at->format('F d, Y \a\t g:i A') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-neutral-500 dark:text-neutral-400">Updated At
                                    </dt>
                                    <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                        {{ $contribution->updated_at->format('F d, Y \a\t g:i A') }}</dd>
                                </div>
                            </dl>
                        </div>
                    </div>
