                        ...
                        <div class="sm:flex sm:items-start">
                            <div class="w-full">
                                <div class="flex justify-between items-center mb-4">
                                    ...
                                    <flux:button variant="primary" icon="x-mark" variant="outline" size="sm"
                                        ...
                                        <div>
                                            ...
                                            <div class="space-y-1 text-sm">
                                                <div><span class="font-medium">Name:</span>
                                                    {{ $contribution->member->full_name }}</div>
                                                <div><span class="font-medium">Registration:</span>
                                                    {{ $contribution->member->registration_no }}</div>
                                                <div><span class="font-medium">Phone:</span>
                                                    {{ $contribution->member->phone ?? 'N/A' }}</div>
                                            </div>
                                        </div>
                                        <div>
                                            ...
                                            <div class="space-y-1 text-sm">
                                                <div><span class="font-medium">Amount:</span>
                                                    ₦{{ number_format($contribution->amount) }}</div>
                                                <div><span class="font-medium">Method:</span>
                                                    {{ $contribution->payment_method_label }}</div>
                                                <div><span class="font-medium">Reference:</span> <span
                                                        class="font-mono">{{ $contribution->payment_reference }}</span>
                                                </div>
                                                <div><span class="font-medium">Date:</span>
                                                    {{ $contribution->payment_date->format('M d, Y') }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="border border-gray-200 rounded-lg p-4">
                                    ...
                                    @if($contribution->has_receipt)
                                        <div class="flex justify-center">
                                            @if($isImage)
                                                <img src="{{ $contribution->receipt_url }}" alt="Payment Receipt"
                                                    class="max-w-full max-h-96 rounded-lg shadow-sm" style="max-height: 500px;" />
                                            @elseif($isPdf)
                                                <div class="w-full">
                                                    <iframe src="{{ $contribution->receipt_url }}"
                                                        class="w-full h-96 rounded-lg border" style="min-height: 500px;"></iframe>
                                                </div>
                                            @else
                                        ...
                                        <div class="mt-4 text-center">
                                            <flux:button variant="outline" href="{{ $contribution->receipt_url }}"
                                                target="_blank">
                                ...
                                <div class="mt-4 bg-blue-50 rounded-md p-4">
                                    ...
                                    <div class="text-sm text-blue-700 space-y-1">
                                        <div><span class="font-medium">Submitted by:</span>
                                            {{ $contribution->uploader->name ?? 'Unknown' }}</div>
                                        <div><span class="font-medium">Submitted on:</span>
                                            {{ $contribution->created_at->format('M d, Y h:i A') }}</div>
                                ...
                                @if($this->canVerify())
                                    <div class="mt-6">
                                        ...
                                        <textarea id="verification_notes" wire:model="verificationNotes"
