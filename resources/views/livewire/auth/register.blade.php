<x-layouts.auth>
    <div x-data="registrationForm({
        ...
        <form method="POST" action="{{ route('register.store') }}" class="space-y-6">
            ...
            <div
                class="rounded-xl border border-neutral-200 bg-white shadow-xs dark:border-neutral-700 dark:bg-neutral-800">
                ...
                <div class="space-y-4 px-4 py-5 sm:p-6">
                    <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
                        <flux:input name="full_name" :label="__('First Name')" type="text"
                            value="{{ old('full_name') }}" required autofocus autocomplete="given-name"
                            :placeholder="__('First name')" />

                        <flux:input name="family_name" :label="__('Last Name')" type="text"
                            value="{{ old('family_name') }}" required autocomplete="family-name"
                            :placeholder="__('Last name')" />
                    </div>

                    <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
                        <flux:input name="date_of_birth" :label="__('Date of Birth')" type="date"
                            value="{{ old('date_of_birth') }}" required max="{{ now()->toDateString() }}" />

                        <flux:select name="marital_status" :label="__('Marital Status')"
                            placeholder="Select marital status" required>
                            <option value="">{{ __('Select marital status') }}</option>
                            <option value="single" @selected(old('marital_status') === 'single')>{{ __('Single') }}
                            </option>
                            <option value="married" @selected(old('marital_status') === 'married')>{{ __('Married') }}
                            </option>
                            <option value="divorced" @selected(old('marital_status') === 'divorced')>{{ __('Divorced') }}
                            </option>
                        </flux:select>
                    </div>

                    <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
                        <flux:input name="nin" :label="__('(NIN)')" type="text" minlength="11" maxlength="11"
                            value="{{ old('nin') }}" required placeholder="11-digit NIN">
                            <x-slot:helperText>
                                {{ __('Enter the 11-digit number on your National ID card.') }}
                            </x-slot:helperText>
                        </flux:input>

                        <flux:input name="hometown" :label="__('Hometown')" type="text" value="{{ old('hometown') }}"
            ...
            <div
                ...
                <div class="space-y-4 px-4 py-5 sm:p-6">
                    <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
                        <flux:input name="phone" :label="__('Phone Number')" type="tel" value="{{ old('phone') }}"
                            required autocomplete="tel" placeholder="+234...">
                            <x-slot:helperText>{{ __('Use a phone number we can call for verification.') }}</x-slot:helperText>
                        </flux:input>

                        <flux:input name="email" :label="__('Email address')" type="email" value="{{ old('email') }}"
                            required autocomplete="email" placeholder="email@example.com" />
                    </div>

                    <flux:textarea name="address" :label="__('Residential Address')" rows="3" required
                        placeholder="Street, city, landmark">{{ old('address') }}</flux:textarea>

                    <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
                        <flux:select name="state_id" :label="__('State of Residence')" placeholder="Select state"
                            x-model="stateId" x-on:change="handleStateChange($event)" required>
                            <option value="">{{ __('Select state') }}</option>
                            @foreach ($states as $state)
                                <option value="{{ $state->id }}" @selected(old('state_id') == $state->id)>{{ $state->name }}
                                </option>
                            @endforeach
                        </flux:select>

                        <flux:select name="lga_id" :label="__('Local Government Area')" placeholder="Select LGA"
                            x-model="lgaId" x-bind:disabled="! availableLgas.length" required>
            ...
            <div
                ...
                <div class="space-y-4 px-4 py-5 sm:p-6">
                    <div class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2">
                        <flux:input name="occupation" :label="__('Occupation')" type="text"
                            value="{{ old('occupation') }}" placeholder="{{ __('Your current role (optional)') }}" />

                        <flux:input name="workplace" :label="__('Place of Work')" type="text"
                            value="{{ old('workplace') }}" placeholder="{{ __('Organisation name (optional)') }}" />
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <div
                    class="border-t border-neutral-200 bg-neutral-50 px-4 py-4 dark:border-neutral-700 dark:bg-neutral-900/40 sm:px-6">
                    <flux:text class="text-sm text-neutral-500 dark:text-neutral-400">
